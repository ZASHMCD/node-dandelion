	...
	***********/
	...
	else
	{
		if (typeof callback == 'function') {
			var err = {
				"success":false,
				"message":"You need to provide your dandelion.eu APP key / APP id pair, our you API token."
			}
			callback(err, null);
			return;
		}
...
/**** authInject****
authInject appends app key / app id or the single token to the uri.
it returns an empty string if the credentials have not been specified.
******************/
var authInject = function(){
	if (dandelionAppId && dandelionAppKey && dandelionAppId !== "" && dandelionAppKey !== "") {
		return `&$app_id=${dandelionAppId}&$app_key=${dandelionAppKey}`;
	} else if(dandelionToken && dandelionToken !== ""){
		return `&token=${dandelionToken}`;
	}
	return '';
}
